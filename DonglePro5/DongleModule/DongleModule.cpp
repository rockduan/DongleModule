...
{
	cout<<"DongleModule::file_exists"<<endl;
	return (access(filename, 0) == 0);
}

int DongleModule::scan_dir(char *dir, int depth)
{
	cout<<"DongleModule::scan_dir()"<<endl;
    DIR *dp;
    int result =0;
    struct dirent *entry;
    struct stat statbuf;
...
void DongleModule::my_progress_callback(int stage, int max, int current) {
	cout<<"DongleModule::my_progress_callback"<<endl;
	SAVE_CURSOR();
	MOVETO(1, 1);
	CLEAR_END();
	MOVETO(1, 10);
...
int DongleModule::findUpdateBinFromUDisk(){
	cout<<" DongleModule::findUpdateBinFromUDisk()"<<endl;
	int result =0;
